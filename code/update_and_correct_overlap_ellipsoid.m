function r_new = update_and_correct_overlap_ellipsoid(f,S,r,bigtri,a,b,c,dr,dt)
% Overlap correction scheme for ellipsoidal map.
%
...
while flag
    ...
    S_rotN = rotate_sphere(f,[S(:,1)/a,S(:,2)/b,S(:,3)/c],bigtri); % as we just want to check the bijectivity, we simply rescale the ellipsoid to a sphere here
    ...
    if north_success
        ...
        S_rotS = rotate_sphere(f,[S(:,1)/a,S(:,2)/b,S(:,3)/c],south_f);
